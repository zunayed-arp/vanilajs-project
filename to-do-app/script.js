...
/* const todos = [
	{
		item: "To complete Python",
		isCompleted: false
	},
	{
		item: "Brain Station 23 Exam",
		isCompleted: true
	}
...
const addTodo = () => {
	...
	todos.forEach(todo => {
		...
		//completed => add respective class //attribute
		...
		cbInput.addEventListener("click", function () {
			...
			//state todos in localstorage

			stateTodo(
				[
					...document.querySelectorAll(".todos .card")
				].indexOf(correspondingCard), checked
			);

			//update class

			checked ? correspondingCard.classList.add("checked") :
				correspondingCard.classList.remove("checked");
		...
		button.addEventListener("click", function () {
			...
			correspondingCard.addEventListener("animationend", function () {
				setTimeout(function () {
					correspondingCard.remove();
					itemLeft.textContent = document.querySelectorAll(".todos .card:not(.checked)"
					).length;
				}, 100);
			});
		});
